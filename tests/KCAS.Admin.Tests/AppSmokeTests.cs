using System.Net;

namespace KCAS.Admin.Tests;

[Collection(KcasTestCollection.Name)]
public sealed class AppSmokeTests(KcasWebApplicationFactory factory)
{
    [Theory]
    [InlineData("/")]
    [InlineData("/Account/Login")]
    [InlineData("/health/live")]
    [InlineData("/health/ready")]
    public async Task Public_pages_return_success(string url)
    {
        var client = factory.CreateClient();

        var response = await client.GetAsync(url);

        response.EnsureSuccessStatusCode();
    }

    [Theory]
    [InlineData("/clients")]
    [InlineData("/clients/new")]
    [InlineData("/clients/1/edit")]
    [InlineData("/clients/1/notes/new")]
    [InlineData("/security")]
    [InlineData("/imports")]
    [InlineData("/investments/summary")]
    [InlineData("/investments/reconciliation")]
    [InlineData("/compliance")]
    [InlineData("/compliance/settings")]
    [InlineData("/compliance/profile")]
    [InlineData("/compliance/governance")]
    [InlineData("/compliance/organisation")]
    [InlineData("/compliance/organisation/update")]
    [InlineData("/compliance/methodologies/update")]
    [InlineData("/compliance/references/update")]
    [InlineData("/compliance/evidence/update")]
    [InlineData("/compliance/tasks/update")]
    [InlineData("/compliance/methodologies")]
    [InlineData("/compliance/methodologies/1/review")]
    [InlineData("/compliance/audit")]
    [InlineData("/compliance/client-evidence")]
    [InlineData("/compliance/client-risk")]
    [InlineData("/compliance/review-transfers")]
    [InlineData("/compliance/business-risk")]
    [InlineData("/compliance/business-risk/1")]
    [InlineData("/compliance/business-risk/1/print")]
    [InlineData("/compliance/rmcp")]
    [InlineData("/compliance/rmcp/1")]
    [InlineData("/compliance/rmcp/1/print")]
    [InlineData("/compliance/programme-transfers")]
    [InlineData("/advice/transfers")]
    [InlineData("/compliance/work-register")]
    [InlineData("/compliance/work-register/1")]
    [InlineData("/compliance/goaml")]
    [InlineData("/compliance/goaml/transfers")]
    [InlineData("/compliance/goaml/checks/1/evidence")]
    [InlineData("/compliance/inspections")]
    [InlineData("/compliance/inspections/1")]
    [InlineData("/compliance/inspections/1/print")]
    [InlineData("/compliance/inspections/1/export.json")]
    [InlineData("/clients/1/evidence")]
    [InlineData("/clients/1/ownership")]
    [InlineData("/clients/1/risk")]
    [InlineData("/clients/1/risk/1")]
    [InlineData("/clients/operational-review")]
    [InlineData("/clients/1/verification")]
    public async Task Protected_pages_redirect_anonymous_users_to_login(string url)
    {
        var client = factory.CreateClient(new() { AllowAutoRedirect = false });

        var response = await client.GetAsync(url);

        Assert.Equal(HttpStatusCode.Redirect, response.StatusCode);
        Assert.StartsWith("http://localhost/Account/Login", response.Headers.Location?.ToString());
    }

    [Fact]
    public async Task Kanaan_header_image_is_served_as_jpeg()
    {
        var client = factory.CreateClient();

        var response = await client.GetAsync("/images/kanaan-header.jpg");

        response.EnsureSuccessStatusCode();
        Assert.Equal("image/jpeg", response.Content.Headers.ContentType?.MediaType);
    }

    [Fact]
    public async Task Kcas_css_contains_shell_rules()
    {
        var client = factory.CreateClient();

        var css = await client.GetStringAsync("/kcas.css");

        Assert.Contains(".top-brand", css);
        Assert.Contains(".sidebar-toggle:checked", css);
    }
}
